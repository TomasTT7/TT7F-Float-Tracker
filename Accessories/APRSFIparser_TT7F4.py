...
for i in range(len(splitlines)):
    ...
    if packet[15] == '|':   # packet without backlog
        ...
        callsign.append(splitcolon[3][1:9])
        year.append(y)
        month.append(m)
        day.append(d)
        hour.append(h)
...
for i in range(len(year)):
    ...
    #print ('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:08b} {:s}' \
    #      .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bit2[i], bit3[i], bit4[i], bit5[i], bit6[i], bit7[i], bitfield[i], backlog[i]))
    dataOUT.append(('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:08b} {:s}' \
          .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bit2[i], bit3[i], bit4[i], bit5[i], bit6[i], bit7[i], bitfield[i], backlog[i])))
...
# CSV
f1 = open('TT7F4data.csv', 'w')
f1.write("""Timestamp,Solar,Battery,MCU,TX,Satellites,Sequence,Des,PSM\n""")
...
# KML
f = open('TT7F4data.kml', 'w')

f.write("""<?xml version="1.0" encoding="UTF-8"?>\n""")
f.write("""<kml xmlns="http://earth.google.com/kml/2.0">\n""")
f.write("""<Document>\n""")
f.write("""   <name>{:s}</name>\n""".format(callsign[0]))
f.write("""   <description>The flight path of TT7F4 superpressure balloon.</description>\n""")
...
for i in range(len(dataORDERED)):
    ...
    f.write("""               <Data name="Descending"><value>{:s}</value></Data>\n""".format(dTemp[18]))
