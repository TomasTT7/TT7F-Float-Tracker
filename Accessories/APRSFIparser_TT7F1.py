...
for i in range(len(year)):
    ...
    #print ('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:08b} {:s}' \
    #      .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bitfield[i], backlog[i]))
    dataOUT.append(('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:08b} {:s}' \
          .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bitfield[i], backlog[i])))
...
# CSV
f1= open('TT7F1data.csv', 'w')
f1.write("""Timestamp,Solar,Supercap,MCU,TX,Satellites,Sequence\n""")

for i in range(len(dataORDERED)):
    
    dT = dataORDERED[i].split(' ')
    f1.write("""{:s}-{:s}-{:s} {:s}:{:s}:{:s},{:s},{:s},{:s},{:s},{:s},{:s}\n""".format(dT[1], dT[2], dT[3], dT[4], dT[5], dT[6], dT[11], dT[12], dT[13], dT[14], dT[15], dT[10]))

f1.close()


# KML
f = open('TT7F1data.kml', 'w')

f.write("""<?xml version="1.0" encoding="UTF-8"?>\n""")
f.write("""<kml xmlns="http://earth.google.com/kml/2.0">\n""")
f.write("""<Document>\n""")
f.write("""   <name>{:s}</name>\n""".format(callsign[0]))
f.write("""   <description>The flight path of TT7F1 superpressure balloon.</description>\n""")
