...
# raw packets: 2017-07-20 20:39:51 CEST: OK7DMT-8>APRS,WIDE2-1,qAR,OK0DSK-1,T2CZECH:!/5L+"S);UOJRW 1G41<5MH:S(L[HF-/;v+:14!'|!,!*<e*w0h!'!$|
# input file: APRSfi_TT7F6W.txt
# Output KML and CSV
...
for i in range(len(year)):
    ...
    #print ('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:08b} {:s}' \
    #      .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bit2[i], bit3[i], bit4[i], bit5[i], bitfield[i], backlog[i]))
    dataOUT.append(('{:s} {:d} {:02d} {:02d} {:02d} {:02d} {:02d} {:.5f} {:.5f} {:.0f} {:d} {:.3f} {:.3f} {:.1f} {:.1f} {:d} {:d} {:d} {:d} {:d} {:d} {:d} {:08b} {:s}' \
          .format(callsign[i], year[i], month[i], day[i], hour[i], minute[i], second[i], latitude[i], longitude[i], altitude[i], sequence[i], vsolar[i], vbattery[i], tempmcu[i], temptx[i], satellite[i], bit0[i], bit1[i], bit2[i], bit3[i], bit4[i], bit5[i], bitfield[i], backlog[i])))
...
# CSV
f1= open('TT7F6Wdata.csv', 'w')
f1.write("""Timestamp,Solar,Supercap,MCU,TX,Satellites,Sequence,Wsp,Urt,Twi,Spi,Altitude\n""")

for i in range(len(dataORDERED)):
    
    dT = dataORDERED[i].split(' ')
    f1.write("""{:s}-{:s}-{:s} {:s}:{:s}:{:s},{:s},{:s},{:s},{:s},{:s},{:s},{:s},{:s},{:s},{:s},{:s}\n""".format(dT[1], dT[2], dT[3], dT[4], dT[5], dT[6], dT[11], dT[12], dT[13], dT[14], dT[15], dT[10], dT[18], dT[19], dT[20], dT[21], dT[9]))

f1.close()


# KML
f = open('TT7F6Wdata.kml', 'w')
...
f.write("""               <![CDATA[
                    ...
                    <B>WSPR:</B> $[WSPR]<br/>
                    <B>UART Error:</B> $[UART Error]<br/>
                    <B>TWI Error:</B> $[TWI Error]<br/>
                    <B>SPI Error:</B> $[SPI Error]<br/>
...
for i in range(len(dataORDERED)):
    ...
    f.write("""               <Data name="WSPR"><value>{:s}</value></Data>\n""".format(dTemp[18]))
    f.write("""               <Data name="UART Error"><value>{:s}</value></Data>\n""".format(dTemp[19]))
    f.write("""               <Data name="TWI Error"><value>{:s}</value></Data>\n""".format(dTemp[20]))
    f.write("""               <Data name="SPI Error"><value>{:s}</value></Data>\n""".format(dTemp[21]))
